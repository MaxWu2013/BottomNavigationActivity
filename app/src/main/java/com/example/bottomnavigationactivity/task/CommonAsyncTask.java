package com.example.bottomnavigationactivity.task;
...
public class CommonAsyncTask {
	...
	public void doConnection(int connectionType, Object... params) {
		if (mTask == null && connectionType >= 0) {
			...
			Log.w("AsyncDataTask: ","doConnection 开启异步任务异常！");
	...
	class DataAsyncTask extends AsyncTask<Object, String, Result> {
		...
		protected Result doInBackground(Object... params) {
			...
			Log.w(TAG, "mDataListener=" + mDataListener);
			try {
		...
		protected void onPostExecute(Result result) {
			...
			try {
				if (null != mDataListener) {
					...
					Log.w("AsyncDataTask : ", "IDataListener为空,请设置.");
				}
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
